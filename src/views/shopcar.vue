<template>
    <div>
        <Head />
        <ShopDataNull v-if="dataNull"/>
        <ShopData v-if="!dataNull"/>
        <Foot v-if="!dataNull"/>
        <GuessLike />
    </div>
</template>
...
import GuessLike from '@/components/shopcar/guessLike';
import Foot from '@/components/shopcar/footer';
export default {
    data(){
        return {
            dataNull:false
    ...
    mounted(){
        this.shopCartList.length?'':this.dataNull= true;
    },
    computed:{
        shopCartList(){
